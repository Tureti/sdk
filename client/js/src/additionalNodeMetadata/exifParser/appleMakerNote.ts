...
export function isAppleMakerNote(note?: unknown): Uint8Array<ArrayBuffer> | undefined {
...
export function detectSelfieFromMakerNote(makerNote?: Uint8Array<ArrayBuffer>): boolean {
...
export function detectPortraitFromMakerNote(makerNote?: Uint8Array<ArrayBuffer>): boolean {
    if (!makerNote || makerNote.length < 20) {
        return false;
    }
...
function findCameraTypeInMakerNote(makerNote?: Uint8Array<ArrayBuffer>): number | null {
    return extractValueFromMakerNote(0x002e, makerNote);
}

function getImageCaptureType(makerNote?: Uint8Array<ArrayBuffer>): number | null {
    return extractValueFromMakerNote(0x0014, makerNote);
}

function extractValueFromMakerNote(tagToFind: number, makerNote?: Uint8Array<ArrayBuffer>): number | null {
    if (!makerNote || makerNote.length < 20) {
        return null;
    }
