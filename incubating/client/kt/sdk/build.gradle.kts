...
android {
    ...
    sourceSets {
        getByName("main") {
            ...
            // Native runtime libraries, generated by buildNativeRuntimeLibs below.
            jniLibs.srcDirs(layout.buildDirectory.dir("cs/jni-libs"))
            proto {
...
tasks.register<Exec>("buildNativeRuntimeLibs") {
    group = "build"
    description = "Generate native libraries, defined in local-jnilibs.sh, when missing or stale."
    // Exec bypasses the log level; route the script through the logger instead.
    logging.captureStandardOutput(LogLevel.LIFECYCLE)
    logging.captureStandardError(LogLevel.ERROR)
    val wrapper = rootProject.layout.projectDirectory.file("scripts/local-jnilibs.sh")
    // The whole dir: the recipe is split across build-native-libs/-openssl/-sqlite
    // and native-libs-common, and a change to any of them changes the output.
    inputs.dir(rootProject.layout.projectDirectory.dir("scripts"))
    // $ABIS selects which ABIs the output holds, so it has to invalidate too.
    inputs.property("abis", providers.environmentVariable("ABIS").orElse(""))
    outputs.dir(layout.buildDirectory.dir("cs/jni-libs"))
    onlyIf { System.getenv("CI") == null }
    commandLine("bash", wrapper.asFile.absolutePath)
}
...
tasks.named { name ->
    ...
    dependsOn("copySharedLibrary")
    dependsOn("buildNativeRuntimeLibs")
}
