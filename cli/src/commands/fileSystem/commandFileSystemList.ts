...
export class CommandFileSystemList implements Command {
    ...
    options: Options = {
        ...
        short: {
            ...
            short: 's',
            default: false,
            help: 'Print only the icon and name of each node.',
        },
    };

    async action({ sdk, photosSdk, paths, args: [pathString], options: { json, type, recursive, short } }: ActionArgs) {
        ...
        const shortOutput = Boolean(short);
        ...
        switch (path.type) {
            ...
            case PathType.MyFiles:
                await this.printChildren(sdk, paths, pathString, { json, nodeType, recursive: Boolean(recursive), short: shortOutput });
                break;
            case PathType.Devices:
                if (path.fullPath === `/${PathType.Devices}`) {
                    await this.printDevices(sdk, { json });
                } else {
                    await this.printChildren(sdk, paths, pathString, { json, recursive: Boolean(recursive), short: shortOutput });
                }
                break;
            case PathType.SharedByMe:
                if (path.fullPath === `/${PathType.SharedByMe}`) {
                    await this.printSharedNodes(sdk, { json, short: shortOutput });
                } else {
            ...
            case PathType.SharedWithMe:
                if (path.fullPath === `/${PathType.SharedWithMe}`) {
                    await this.printSharedWithMe(sdk, { json, short: shortOutput });
                } else {
                    await this.printChildren(sdk, paths, pathString, { json, recursive: Boolean(recursive), short: shortOutput });
                }
                break;
            case PathType.Trash:
                if (path.fullPath === `/${PathType.Trash}`) {
                    await this.printTrashedNodes(sdk, { json, short: shortOutput });
                } else {
            ...
            case PathType.PhotosSharedByMe:
                if (path.fullPath === `/${PathType.PhotosSharedByMe}`) {
                    await this.printSharedNodes(photosSdk, { json, short: shortOutput });
                } else {
                    throw new ValidationError(`Use direct path to list children of shared node`);
                }
                break;
            case PathType.PhotosSharedWithMe:
                if (path.fullPath === `/${PathType.PhotosSharedWithMe}`) {
                    await this.printSharedWithMe(photosSdk, { json, short: shortOutput });
                } else {
            ...
            case PathType.PhotosTrash:
                if (path.fullPath === `/${PathType.PhotosTrash}`) {
                    await this.printTrashedNodes(photosSdk, { json, short: shortOutput });
                } else {
    ...
    private async printChildren(
        ...
        paths: Paths,
        pathString: string,
        options: { json: boolean; nodeType?: NodeType; recursive?: boolean; short?: boolean },
    ) {
        ...
        await printIterable(childrenIterator, options.json, (node) => this.printNodeHuman(node, options.short));
    }
    ...
    private async printSharedNodes(
        sdk: ProtonDriveClient | ProtonDrivePhotosClient,
        options: { json: boolean; short?: boolean },
    ) {
        await printIterable(sdk.iterateSharedNodes(), options.json, (node) => this.printNodeHuman(node, options.short));
    }

    private async printSharedWithMe(
        sdk: ProtonDriveClient | ProtonDrivePhotosClient,
        options: { json: boolean; short?: boolean },
    ) {
        await printIterable(sdk.iterateSharedNodesWithMe(), options.json, (node) => this.printNodeHuman(node, options.short));
    }

    private async printTrashedNodes(
        sdk: ProtonDriveClient | ProtonDrivePhotosClient,
        options: { json: boolean; short?: boolean },
    ) {
        await printIterable(sdk.iterateTrashedNodes(), options.json, (node) => this.printNodeHuman(node, options.short));
    }

    private printNodeHuman(node: NodeEntity, short = false): void {
        const type = node.type === 'file' ? '📄' : '🗂️';
        const name = getName(node);
        const depth = (node as { treeDepth?: number }).treeDepth ?? 0;

        if (short) {
            console.log('  '.repeat(depth) + sanitizeTerminalText(`${type} ${name}`));
            return;
        }
